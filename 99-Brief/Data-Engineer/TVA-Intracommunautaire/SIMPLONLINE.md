# Champs Simplonline — Valider un référentiel de TVA intracommunautaire
...
> Le brief complet reste `BRIEF_TVA.md` (rendu GitHub).

## Compétences et niveaux

— C8. Automatiser l'extraction de données depuis un service web → Niveau 3, TRANSPOSER
— C10. Développer des règles d'agrégation et d'homogénéisation → Niveau 2, ADAPTER
— C11. Créer une base de données → Niveau 2, ADAPTER
— C12. Partager le jeu de données via une API REST → Niveau 2, ADAPTER

## Titre  (70 / 100)

Valider un référentiel de TVA intracommunautaire — API de vérification

## Description rapide  (658 / 900)

Meridian Distribution facture hors taxe ses clients européens, ce qui n'est légal que si leur numéro de TVA est réellement valide. Le référentiel compte 10 000 numéros accumulés depuis huit ans par trois canaux qui ne se sont jamais parlé, et personne ne les a jamais vérifiés. Le dernier contrôle fiscal a coûté cher.

Vous construisez le service qui tranche, et vous produisez l'état des lieux que la direction financière attend. Un module de validation hors ligne vous est fourni ; établir ce qu'il garantit réellement, et obtenir ce qui lui manque, est le travail. Attendez-vous à ce que la réalité soit moins binaire que la question ne le laisse croire.
...
— Prérequis : Cours Python, Cours FastAPI et Cours Docker (liens en Ressources)
— Données : 10 000 numéros fournis dans le kit de démarrage
...
— Le jeu de 10 000 lignes, en CSV et en Excel (mêmes données) ;
— le module de **validation structurelle** des dix pays du jeu, avec ses fonctions de normalisation et de validation. Lisez-le : vous devrez expliquer en soutenance ce que « structurellement valide » recouvre, et pourquoi cela ne suffit pas ;
— un fichier de composition Docker pour PostgreSQL, pour ne pas y passer la matinée.

Le service VIES est public et gratuit, sans inscription. Aucun client VIES, aucun schéma de base et aucune API ne sont fournis : c'est le sujet du brief.

## Modalités pédagogiques  (4407 / 6000)
...
**Résultat testable en fin de J2 :** une campagne s'exécute en mode échantillon, chaque ligne porte un verdict que vous savez justifier, une relance ne refait pas les appels déjà faits, l'API renvoie verdict, origine et fraîcheur, et le rapport se régénère par une commande.

## Modalités d'évaluation  (696 / 3000)
...
## Critères de performance  (1550 / 3000)

**C8 — Automatiser l'extraction depuis un service web — Niveau 3, TRANSPOSER**

— Le client VIES est fonctionnel et une campagne aboutit sur le volume annoncé.
— Le modèle de verdicts rend compte de **tous** les cas que le service renvoie, y compris ceux où il ne permet pas de conclure. **Aucune ligne n'est déclarée invalide sans preuve** : un défaut du service n'est pas un défaut du numéro.
...
**C10 — Homogénéiser et fiabiliser le jeu — Niveau 2, ADAPTER**
...
**C12 — Partager le jeu via une API REST — Niveau 2, ADAPTER**
...
— [Cours FastAPI](https://github.com/gsoulat/formation-data-IA/tree/main/01-Fondamentaux/Python/08-FastAPI)
