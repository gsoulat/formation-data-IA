# Champs Simplonline — Mini-brief 2 : Exposer un catalogue
...
> Le brief complet reste `BRIEF_API.md` (rendu GitHub).

## Compétences et niveaux

— C12. Partager le jeu de données en développant une API REST → Niveau 3, TRANSPOSER
— C21. Implémenter les règles de gouvernance des données, volet contrôles d'accès → Niveau 2, ADAPTER

## Titre  (70 / 100)

Mini-brief 2 : Exposer un catalogue — API REST, rôles et quota d'appel

## Description rapide  (685 / 900)

Le catalogue du concurrent est en base : vous l'y avez mis en début de semaine. Trois personnes chez Bouquineo en ont besoin et aucune ne sait écrire une requête SQL — l'équipe commerciale veut consulter, la responsable des achats veut corriger, la direction veut ouvrir une partie du catalogue à un partenaire extérieur.

Vous exposez le catalogue derrière une API REST. Le sujet n'est pas d'écrire quatre routes : c'est de décider qui a le droit de faire quoi, et de tenir cette décision. Vous passez aussi de l'autre côté du guichet — après avoir envoyé mille requêtes à un serveur en J1-J2, c'est vous qui encaissez, et à vous de vous protéger d'un appelant qui ne se modérera pas.
...
— Prérequis : Cours FastAPI et Cours Docker (liens en Ressources)
— Amont : Mini-brief 1, Scraping, réalisé en J1-J2 : vous exposez le catalogue que vous avez collecté
...
Votre base issue du mini-brief 1, et un `docker-compose.yml` de départ dans le dossier `starter-kit/` du brief.
...
## Modalités pédagogiques  (5619 / 6000)
...
Implémentez le modèle décidé hier. Le mécanisme est votre choix — clé d'API dans un en-tête, ou jeton JWT — mais justifiez-le dans le README ; le cours FastAPI couvre les deux ainsi que les rôles.
...
Soignez surtout ce que voit celui qui dépasse. Le bon comportement est connu : un code **429**, un en-tête `Retry-After` qui dit combien de temps patienter, et des en-têtes annonçant la limite et le solde restant, pour que le client se régule **avant** d'être bloqué. Beaucoup d'API publiques font l'inverse et renvoient un 200 avec l'échec enfoui dans le corps ; un client naïf enregistre alors du vide sans qu'aucune alerte ne se déclenche. Vous avez été ce client il y a trois jours — ne reproduisez pas le piège.
...
## Modalités d'évaluation  (874 / 3000)
...
— Le **fichier de composition** permettant de lancer base et API ensemble, et un fichier `.env.example` sans aucun secret réel.
...
## Critères de performance  (1621 / 3000)

**C12 — Partager le jeu de données via une API REST — Niveau 3, TRANSPOSER**
...
— Le quota est effectif et différencié selon le rôle ; un dépassement renvoie un code 429 accompagné d'un en-tête `Retry-After` exploitable par le client.
...
**C21 — Implémenter les règles de gouvernance, volet contrôles d'accès — Niveau 2, ADAPTER**
...
— Le mécanisme de persistance des corrections est implémenté, documenté et démontré en direct : une correction saisie via l'API survit à un rechargement du catalogue, et le sort d'un livre ajouté manuellement absent du jeu rechargé est traité et justifié.

## Ressources  (997 car.)

— [Cours FastAPI](https://github.com/gsoulat/formation-data-IA/tree/main/01-Fondamentaux/Python/08-FastAPI), dont [authentification, JWT et rôles](https://github.com/gsoulat/formation-data-IA/blob/main/01-Fondamentaux/Python/08-FastAPI/06-authentification.md)
— [Cours Docker](https://github.com/gsoulat/formation-data-IA/tree/main/02-Containerisation/Docker)
— [Mini-brief 1, l'amont de celui-ci](https://github.com/gsoulat/formation-data-IA/blob/main/99-Brief/Data-Engineer/Bouquineo-Scraping/BRIEF_SCRAPING.md)
